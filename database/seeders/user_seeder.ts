...
export default class extends BaseSeeder {
  async run() {
    ...
    const guestRole = await Role.findByOrFail('slug', 'guest')

    // Assign admin role to admin user (override default 'user' role)
    ...
    await guestUser.related('roles').sync([guestRole.id])
  }
}
